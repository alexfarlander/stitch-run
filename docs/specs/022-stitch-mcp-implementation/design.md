...
**File**: `src/lib/api/mcp-auth.ts` (new file in main Stitch app)

Validates MCP server requests using the shared API key.
...
export function validateMCPAuth(request: Request): boolean {
  ...
  return token === process.env.STITCH_API_KEY;
}

export function requireMCPAuth(handler: Function) {
...
**Usage**: Wrap MCP-specific API endpoints with `requireMCPAuth` middleware.

### Code Template Generator

**File**: `src/lib/templates.ts` (optional - can be inline in get-stitching-code.ts)

Generates framework-specific integration code snippets for external assets. This can be implemented as a separate module for better organization or kept inline in the tool handler for simplicity.
...
**Implementation Note**: For V1, templates can be inline string literals in `get-stitching-code.ts`. Extract to separate file if templates become complex or numerous.
...
- `STITCH_API_KEY`: New API key for authenticating MCP server requests (should match the key configured in Claude Desktop). Generate using: `openssl rand -hex 32`
- `NEXT_PUBLIC_BASE_URL`: Base URL for generating webhook and uptime URLs

**Setting up API Key**:
1. Generate a secure random key: `openssl rand -hex 32`
2. Add to `.env.local` in Stitch app: `STITCH_API_KEY=your-generated-key`
3. Add same key to Claude Desktop MCP config
4. Restart both Stitch app and Claude Desktop

## Security Considerations

### Authentication Strategy

The MCP server uses a simple API key authentication mechanism separate from Supabase auth:

1. **STITCH_API_KEY**: A new environment variable that acts as a shared secret between the MCP server and Stitch platform
2. **Implementation**: Create a new middleware in `src/lib/api/auth.ts` that validates the Bearer token against `process.env.STITCH_API_KEY`
3. **Scope**: This auth is specifically for MCP server requests, not for end-user authentication
4. **Generation**: API key should be a secure random string (e.g., UUID or 32-byte hex string)

**Why separate from Supabase auth?**
- MCP server is a system-level integration, not a user
- Simpler to configure in Claude Desktop
- Avoids complexity of managing Supabase service role keys in client config

### Additional Security Measures

1. **API Key Management**: API keys should be stored securely and never committed to version control
2. **Webhook Validation**: Consider adding HMAC signature validation for webhook requests in future versions
